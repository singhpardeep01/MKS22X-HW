...
public class MyHeap<T extends Comparable<T>>
{
	...
	while (k*2 <= size && k*2+1 <= size) {
	    ...
	    if (data[k*2].compareTo(data[k])*max> 0 || data[k*2+1].compareTo(data[k])*max > 0) {
		if (data[k*2].compareTo(data[k*2+1])*max > 0) {
		    T temp = data[k];
		    data[k] = data[k*2];
		    data[k*2] = temp;
		    k *= 2;
	...
	if (k*2 <= size && data[k*2].compareTo(data[k])*max > 0) {
	    T temp = data[k];
	    data[k] = data[k*2];
	    data[k*2] = temp;
	    k *= 2;
	...
	while (k > 1) {
	    ...
	    if (data[k].compareTo(data[k/2])*max > 0) {
		T temp = data[k];
		data[k] = data[k/2];
		data[k/2] = temp;
		k = k/2;
	...
	MyHeap<Integer> a = new MyHeap<Integer>(false);
	a.add(5);
	a.add(4);
	a.add(3);
	a.add(2);
	...
	MyHeap<Integer> c = new MyHeap<Integer>(b,false);
